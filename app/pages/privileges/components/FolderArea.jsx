...
export default class FolderArea extends React.Component {
  ...
  static propTypes = {
    ...
    selectFolders: React.PropTypes.func,
  }
    ...
    return (
      <Content style={styles.content}>
        ...
        <div>
          <Tree
            checkable
            onCheck={selectFolders}
            checkedKeys={selectedGroup.folderIds ? selectedGroup.folderIds.map(item => item + '') : []}>
      ...
      if (rest.length)
        return (
          <TreeNode
            key={item.id}
            title={item.name}
            disableCheckbox={!(group.workgroupId && group.roleId)}>
